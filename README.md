...
3. 验证工具可用：
```bash
npx --yes wechat-md-publisher@^1.0.2 --version
```
...
# 发布
npx --yes wechat-md-publisher@^1.0.2 publish create --file test.md --theme default
```
...
# 或直接使用工具
npx --yes wechat-md-publisher@^1.0.2 publish create --file article.md --theme orangeheart
```
...
```bash
# 添加本地主题（推荐）
npx --yes wechat-md-publisher@^1.0.2 theme add-local --name my-theme --path ./my-theme.css

# 添加远程主题 API（⚠️ 见下方安全警告）
npx --yes wechat-md-publisher@^1.0.2 theme add-remote --name md2wechat --url https://api.md2wechat.cn --key xxx
```
...
Token 会自动刷新，如仍有问题：
```bash
npx --yes wechat-md-publisher@^1.0.2 account remove <account-id>
npx --yes wechat-md-publisher@^1.0.2 account add --name "新账号" --default
```
